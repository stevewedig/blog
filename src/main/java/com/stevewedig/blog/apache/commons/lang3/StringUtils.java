 ...
 * <li><b>AppendIfMissing</b> - appends a suffix to the end of the String if not present</li>
 * <li><b>PrependIfMissing</b> - prepends a prefix to the start of the String if not present</li>
 ...
 * <li><b>LevenshteinDistance</b> - the number of changes needed to change one String into another</li>
 * </ul>
 * 
 * <p>
 ...
 * 
 * <p>
 * {@code StringUtils} handles {@code null} input Strings quietly. That is to say that a
 * {@code null} input will return {@code null}. Where a {@code boolean} or {@code int} is being
 * returned details vary by method.
 * </p>
 * 
 * <p>
 * A side effect of the {@code null} handling is that a {@code NullPointerException} should be
 * considered a bug in {@code StringUtils}.
 * </p>
 * 
 * <p>
 * Methods in this class give sample code to explain their operation. The symbol {@code *} is used
 * to indicate any input including {@code null}.
 * </p>
 * 
 * <p>
 * #ThreadSafe#
...
public abstract class StringUtils {
  ...
  // =========================================================================
  // gwt compatible version of Character.isWhitespace
  // =========================================================================

  // https://gwt.googlesource.com/gwt/+/master/user/super/com/google/gwt/emul/java/lang/Character.java
  ...
  // Whitespace:
  // isWhitespace() is faster than WHITESPACE.indexOf()
  ...
  /**
   * <p>
   * {@code StringUtils} instances should NOT be constructed in standard programming. Instead, the
   * class should be used as {@code StringUtils.trim(" foo ");}.
   * </p>
   * 
   * <p>
  ...
  public static boolean isBlank(final CharSequence cs) {
    ...
    for (int i = 0; i < strLen; i++) {
      if (isWhitespace(cs.charAt(i)) == false) {
        return false;
      }
    }
    return true;
   ...
   * {@code null} stripChars will strip whitespace as defined by
   * {@link Character#isWhitespace(char)}.
   * </p>
   * 
   * <pre>
...
//  }
   ...
   * 
   * <p>
   * A {@code null} or empty ("") CharSequence will return {@code (INDEX_NOT_FOUND) -1}. A negative
   * start position is treated as zero. A start position greater than the string length returns
   * {@code -1}.
   * </p>
   * 
   * <pre>
   ...
   */
  public static int ordinalIndexOf(final CharSequence str, final CharSequence searchStr,
      final int ordinal) {
   ...
   */
  public static int lastOrdinalIndexOf(final CharSequence str, final CharSequence searchStr,
      final int ordinal) {
   ...
   * negative. A start position greater than the string length searches the whole string. The search
   * starts at the startPos and works backwards; matches starting after the start position are
   * ignored.
   * </p>
   * 
   * <pre>
...
//  }
   ...
   * 
   * <p>
   * No delimiter is added before or after the list. Null objects or empty strings within the array
   ...
   */
  public static String replaceOnce(final String text, final String searchString,
...
//  }
   ...
   * 
   * <p>
   * For a word based algorithm, see org.apache.commons.lang3.text.WordUtils#uncapitalize(String). A
   * {@code null} input String returns {@code null}.
   * </p>
   * 
   * <pre>
   ...
   * 
   * <p>
   * For a word based algorithm, see org.apache.commons.lang3.text.WordUtils#swapCase(String). A
   * {@code null} input String returns {@code null}.
   * </p>
   * 
   * <pre>
...
//  }
   ...
   * Chas Emerick's implementation of the Levenshtein distance algorithm from <a
   * href="http://www.merriampark.com/ld.htm">http://www.merriampark.com/ld.htm</a>
   * </p>
   * 
   * <pre>
  ...
  public static int getLevenshteinDistance(CharSequence s, CharSequence t, final int threshold) {
    ...
    /*
     * This implementation only computes the distance if it's less than or equal to the threshold
     * value, returning -1 if it's greater. The advantage is performance: unbounded distance is
     * O(nm), but a bound of k allows us to reduce it to O(km) time by only computing a diagonal
     * stripe of width 2k + 1 of the cost table. It is also possible to use this to compute the
     * unbounded Levenshtein distance by starting the threshold at 1 and doubling each time until
     ...
     * |#|#|#|* d[] = *|#|#|#| We must ignore the entry to the left of the leftmost member We must
     * ignore the entry above the rightmost member
     * 
     * Another subtlety comes from our stripe running off the matrix if the strings aren't of the
     * same size. Since string s is always swapped to be the shorter of the two, the stripe will
     * always run off to the upper right instead of the lower left of the matrix.
     * 
     * As a concrete example, suppose s is of length 5, t is of length 7, and our threshold is 1. In
     * this case we're going to walk a stripe of length 3. The matrix would look like so:
     * 
     ...
     * arrays and swapping them back and forth instead of allocating an entire n by m matrix. This
     * requires a few minor changes, such as immediately returning when it's detected that the
     * stripe has run off the matrix and initially filling the arrays with large values so that
  ...
  public static double getJaroWinklerDistance(final CharSequence first, final CharSequence second) {
    ...
    final double matchScore =
        Math.round((jaro + (DEFAULT_SCALING_FACTOR * cl * (1.0 - jaro))) * 100.0) / 100.0;
  ...
  private static double score(final CharSequence first, final CharSequence second) {
    ...
    final double dist =
        (m1.length() / ((double) shorter.length()) + m2.length() / ((double) longer.length()) + (m1
            .length() - transpositions) / ((double) m1.length())) / 3.0;
    return dist;
  }
   ...
   * TextMate, Atom and others. One point is given for every matched character. Subsequent matches
   * yield two bonus points. A higher score indicates a higher similarity.
   * </p>
   * 
   * <pre>
   ...
   * @param locale This string matching logic is case insensitive. A locale is necessary to
   *        normalize both Strings to lower case.
   * @return result score
   * @throws IllegalArgumentException if either String input {@code null} or Locale input
   *         {@code null}
   * @since 3.4
   */
...
//  }
   ...
   */
  private static String getSetOfMatchingCharacterWithin(final CharSequence first,
      final CharSequence second, final int limit) {
    ...
    for (int i = 0; i < first.length(); i++) {
      ...
      // See if the character is within the limit positions away from the original position of that
      // character.
   ...
   * @return {@code true} if the CharSequence starts with the prefix or both {@code null}
   */
  private static boolean endsWith(final CharSequence str, final CharSequence suffix,
      final boolean ignoreCase) {
   ...
   * <p>
   * Similar to <a
   * href="http://www.w3.org/TR/xpath/#function-normalize-space">http://www.w3.org/TR/xpath
   * /#function-normalize -space</a>
   * </p>
   * <p>
   * The function returns the argument string with whitespace normalized by using
   * <code>{@link #trim(String)}</code> to remove leading and trailing whitespace and then replacing
  ...
  public static String normalizeSpace(final String str) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      boolean isWhitespace = isWhitespace(actualChar);
      if (!isWhitespace) {
   ...
   */
  private static String appendIfMissing(final String str, final CharSequence suffix,
      final boolean ignoreCase, final CharSequence... suffixes) {
   ...
   */
  private static String prependIfMissing(final String str, final CharSequence prefix,
      final boolean ignoreCase, final CharSequence... prefixes) {
